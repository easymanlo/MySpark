...
/**
  * Driver HA ：
  * 1.在提交application的时候  添加 --supervise 选项
  * 如果Driver挂掉 会自动启动一个Driver
...
object SparkStreamingDriverHA {
  ...
  def main(args: Array[String]): Unit = {
    ...
    /**
      ...
      *   这个方法首先会从ckDir目录中获取StreamingContext
      *   【 因为StreamingContext是序列化存储在Checkpoint目录中，恢复时会尝试反序列化这些objects。
