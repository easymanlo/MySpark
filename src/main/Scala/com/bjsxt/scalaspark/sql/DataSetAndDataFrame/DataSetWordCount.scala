...
/**
  * DataSet  与RDD类似，它没有使用java 序列化或者Kryo序列化，
  * 而是使用专门的编码器序列化对象，以便对象通过网络进行处理或传输。
  * 虽然编码器和标准序列化负责将对象转换为字节，但编码器是动态生成的，
  * 并使用允许Spark执行许多操作(如过滤、排序和哈希)的格式，而无需将字节反序列化为对象。
...
//    spark.sql("select word+'aaa',count(word) as totalCount from myWords group by word order by totalCount desc").show()
