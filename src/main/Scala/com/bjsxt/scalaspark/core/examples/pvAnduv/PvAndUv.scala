...
//    lines.map(one=>{(one.split("\t")(5),1)}).reduceByKey((v1,v2)=>{v1+v2}).sortBy(tp=>{tp._2},false).foreach(println)
    //uv
    val distinctTrans: RDD[String] = lines.map(one=>{s"${one.split("\t")(0)}_${one.split("\t")(5)}"}).distinct()
//    distinctTrans.foreach(println)
   distinctTrans.map(one=>{(one.split("_")(1),1)}).reduceByKey(_+_).sortBy(_._2).foreach(println)
    //计算每个网址 最活跃的网址及对应的人数
   val groupTrans: RDD[(String, Iterable[String])] = lines.map(one=>{(one.split("\t")(5),one.split("\t")(1))}).groupByKey()
    groupTrans.map(tp=>{
      val site = tp._1
       val localIterator: Iterator[String] = tp._2.iterator
